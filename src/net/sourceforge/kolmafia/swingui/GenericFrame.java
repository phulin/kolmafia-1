...
public abstract class GenericFrame extends JFrame implements Runnable, FocusListener {
  ...
  public void removeHotKeys() {
    ...
    this.getRootPane()
        .unregisterKeyboardAction(KeyStroke.getKeyStroke(KeyEvent.VK_W, KeyEvent.META_DOWN_MASK));
    this.getRootPane()
        .unregisterKeyboardAction(KeyStroke.getKeyStroke(KeyEvent.VK_W, KeyEvent.CTRL_DOWN_MASK));
  ...
  private class TabForwardListener implements ActionListener {
    ...
    public void actionPerformed(final ActionEvent e) {
      ...
      GenericFrame.this.tabs.setSelectedIndex(
          (GenericFrame.this.tabs.getSelectedIndex() + 1) % GenericFrame.this.tabs.getTabCount());
    }
  }
  ...
  private class TabBackwardListener implements ActionListener {
    ...
    public void actionPerformed(final ActionEvent e) {
      ...
      GenericFrame.this.tabs.setSelectedIndex(
          GenericFrame.this.tabs.getSelectedIndex() == 0
  ...
  /**
   * Overrides the default behavior of dispose so that the frames are removed from the internal list
   * of existing frames. Also allows for automatic exit.
   */
  @Override
  public void dispose() {
  ...
  /**
   * Method which adds a compact pane to the west side of the component. Note that this method can
   * only be used if the KoLFrame on which it is called has not yet added any components. If there
   * are any added components, this method will do nothing.
   */
  ...
  /**
   * Overrides the default isEnabled() method, because the setEnabled() method does not call the
  ...
  private void restorePosition() {
    ...
    if (position == null || !position.contains(",")) {
      ...
      if (!(this instanceof OptionsFrame)
    ...
    if (xLocation > display.x
        && yLocation > display.y
        && xLocation < (display.x + display.width)
        && yLocation < (display.y + display.height)) {
    ...
    if (bookmarkData.length > 1) {
      for (int i = 0; i < bookmarkData.length; ++i) {
        KoLConstants.bookmarks.add(
            bookmarkData[i] + "|" + bookmarkData[++i] + "|" + bookmarkData[++i]);
      }
    }
  }

  private static class FramePanel extends JPanel implements FocusListener {
